  ...
  const renderExpense = ({ item }) => (
    <View style={styles.expenseCard}>
      ...
      {Object.entries(item.splits).map(([email, share]) => (
        <Text key={email}> - {email}: ${share.toFixed(2)}</Text>
      ))}
      ...
      <ScrollView>
        ...
        {Object.entries(balances).map(([email, balance]) => (
          <Text key={email} style={styles.balanceText}>
            {email}: {balance < 0 ? `You owe $${Math.abs(balance).toFixed(2)}` : `You are owed $${balance.toFixed(2)}`}
          </Text>
        ))}
...
const styles = StyleSheet.create({
  ...
  groupTitle: { fontSize: 22, fontWeight: 'bold', marginBottom: 10 },
  sectionTitle: { fontSize: 18, fontWeight: '600', marginTop: 16 },
  balanceText: { fontSize: 16, marginVertical: 4 },
  expenseCard: {
    padding: 12,
    marginVertical: 6,
    borderRadius: 8,
